...
function displayResults(results) {
  ...
  // Вставляем кнопку сортировки один раз в верхнюю часть offer-list
  const sortButton = `
    <button id="sort-rating-btn" style="margin: 10px 20px; padding: 5px 10px; background-color: #4CAF50; color: white; border: none; cursor: pointer; font-size: 14px; float: right;">
      Сортировать по сложности
    </button>
  `;

  offerList.innerHTML = sortButton + results
    .map((result, index) => {
  ...
  document.getElementById("sort-rating-btn").addEventListener("click", function () {
    ...
    if (!isFilteredByRating) {
      filteredResults = results.filter(result => result.rating > 0); // Фильтруем по рейтингу > 0
      isFilteredByRating = true;
    } else {
      filteredResults = results; // Сбрасываем фильтрацию
      isFilteredByRating = false;
    }
    // Обновляем отображение предложений
